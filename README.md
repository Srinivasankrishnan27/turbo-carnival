...
The core of the pipeline is the concept of "evaluators." Each evaluator is a self-contained unit responsible for a single metric (e.g., ROUGE score, semantic similarity). This design allows for new evaluation metrics to be added without modifying the core pipeline logic. Evaluators are organized into categories (e.g., `ngram`, `semantic`) for better organization.
